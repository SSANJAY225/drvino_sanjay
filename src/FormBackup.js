    ...
    <div className='d-flex justify-content-center align-items-center Box'>
      <div className="bg-white box">
        ...
        <div className="button-container">
          ...
          <button className='sap btn btn-danger btn-danger' onClick={handleDelete}>Delete</button>
          <button onClick={generatePDF}>Generate Order PDF</button>
        </div>
      </div>
      <PDFPreviewModal
        show={showModal}
...
const PDFPreviewModal = ({ show, handleClose, handleDownload, handleShare, pdfContent, noteText }) => {
  return (
    <Modal className="pdf-modal" show={show} onHide={handleClose} centered size="lg">
      <Modal.Header closeButton>
        <Modal.Title>PDF Preview</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        {/* Use iframe or embed for better compatibility */}
        <iframe src={pdfContent} title="PDF Preview" style={{ width: '100%', height: '500px', border: 'none' }} />
        
        {/* Display the note text in a responsive div */}
        <div style={{ width: '100%', marginTop: '10px', whiteSpace: 'pre-wrap', wordWrap: 'break-word' }}>
          {noteText}
        </div>
      </Modal.Body>
      <Modal.Footer>
